...
<!-- TOOD: Remove one of the following paragraphs. -->

In addition, the command will update the `.cargo/config.toml` to use the prefetched
...
to build your project. The following command will update the `.cargo/config.toml`
to use them. (If the file does not exist, it will be created).
